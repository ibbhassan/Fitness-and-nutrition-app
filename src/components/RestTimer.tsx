...
export const RestTimer: React.FC<RestTimerProps> = ({ lastCompletedSetTime }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[60] bg-tactical-900 border-t border-neon-blue shadow-[0_-5px_30px_rgba(0,240,255,0.15)] pb-safe-area">
      <div className="max-w-4xl mx-auto px-2 sm:px-4 py-3 flex flex-wrap sm:flex-nowrap items-center justify-between gap-y-3 gap-x-1 sm:gap-3">
        {/* Left side: Icon & Timer */}
        <div className="flex items-center gap-2 sm:gap-4 order-1">
          <Timer className={clsx("w-5 h-5 sm:w-6 sm:h-6 hidden sm:block", isActive ? "text-neon-blue animate-pulse" : "text-gray-500")} />
          <div 
